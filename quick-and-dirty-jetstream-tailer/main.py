    ...
    while True:
      try:
        ...
        if message['time_us']:
            ...
            if not (recent_timestamp is None or current_time > recent_timestamp):
...
async def periodic_disk_writer():
    while True:
        ...
        # Write your timestamp to disk here
        with open('last_run_timestamp.txt', 'w') as f:
            f.write(str(recent_timestamp))  # however you access your latest timestamp
...
async def main():
    ...
    if os.path.exists("last_run_timestamp.txt"):
        with open('last_run_timestamp.txt', 'r') as file:
            ...
            cursor = int(file.read()) - 1000*1000*60*60
    else:
